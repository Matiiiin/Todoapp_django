...
class LoginSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not authenticated_user:
            raise serializers.ValidationError({'detail':'Invalid credentials , maybe password or email is incorrect'})
        if not authenticated_user.is_verified:
            raise serializers.ValidationError({'detail':'Provided user is not verified'})
...
class JWTCreateSerializer(serializers.Serializer):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.fields["password"] = serializers.CharField(write_only=True , max_length=255)
    def validate(self, attrs):
        data = super().validate(attrs)
        username_field = data.get(self.username_field)
        password = data.get('password')
        authenticate_user = authenticate(username=username_field , password=password)
...
class JWTRefreshSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        data =  super().validate(attrs)
        try:
            payload = jwt.decode(jwt=attrs.get('refresh'), key=settings.SECRET_KEY, algorithms=['HS256'])
            if not get_object_or_404(User , pk=payload.get('user_id')).is_verified:
                raise serializers.ValidationError({'detail':'The user for provided token is not verified'})
...
class ForgetPasswordConfirmSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        data = super().validate(attrs)
        url_token = self.context.get('request').resolver_match.kwargs.get('token')
        token = jwt.decode(url_token, settings.SECRET_KEY, algorithms=['HS256'])
        ...
        if not user.check_password(attrs.get('temporary_password')):
            raise serializers.ValidationError({'detail':'Temporary password is incorrect'})
        data['user'] = user
        return data
