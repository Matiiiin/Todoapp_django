...
def send_email(ch, method, properties, body):
    try:
        ...
        if not recipients or not email_data.get('sender') or not email_data.get('email_body'):
        ...
        with smtplib.SMTP(host=SMTP_SERVER, port=SMTP_PORT) as server:
            server.send_message(message, from_addr=email_data['sender'], to_addrs=recipients)
